...
#' @description The dependent variable could be generate for more than 100 distributions    which have at maximum 4 parameters. Each parameter could be defined in terms of covariates and random-effects.
#' @description The dependent variable, when normal, could include both an additional error-term and heteroschedasticity.
...
#' @param Dis Type of distribution in form of rDis where Dis is one of the distribution allowe by GAMLSS. It is to be defined to use distribution different from the nornal one
...
#' @param costh A costant used to generate heteroschedasticity
#' @param seed The seed to be used
#' @param id Unit id.
#' @import gamlss
#' @import dplyr
#' @import splitstackshape
#' @return A list of data frame. Each one has: the dependent variable y, a set of fixed covariates, the small area column (sa) and the id column (id)
#' @export
#' @note The definition of the heteroschedastic term follows Ramirez-Aldana, R., & Naranjo, L. (2021).
#' @examples #Normal data
#' @examples data_gen(Ni=rep(10,4), D=4, M=2 ,ty ="no", k=100, b1=4,
#' @examples          x1=rnorm(40, 0,1),  b2 = NULL, x2 = NULL, b3 = NULL,
#' @examples          x3 = NULL, b4 = NULL, x4 = NULL, xh = NULL,
#' @examples          Dis, l=c(identity), sigma = 6,sigmah = NULL,
#' @examples          sigmae = 22,  costh = NULL, seed = 1234)
#' @examples #
#' @examples #Heteroschedastic Normal data
#' @examples #
#' @examples data_gen(Ni=rep(10,4), D=4, M=2 ,ty ="het", k=100, b1=4,
#' @examples          x1=rnorm(40, 0,1), b2 = NULL, x2 = NULL, b3 = NULL,
#' @examples          x3 = NULL, b4 = NULL, x4 = NULL, xh = rnorm(40, 0,1),
#' @examples          Dis, l=c(identity), sigma = 6, sigmah = 1, sigmae = 22,
#' @examples          costh = 0.1, seed = 1234)
#' @examples #
#' @examples #Non-normal data:
#' @examples #
#' @examples #a) Log-normal data
#' @examples #
#' @examples data_gen(Ni=rep(10,4), D=4, M=2 ,ty =NULL, k=c(7,-2), b1=1,
#' @examples          x1=rnorm(40, 0,1), b2 = 0.5, x2 = rnorm(40, 0,1),
#' @examples          b3 = NULL, x3 = NULL, b4 = NULL, x4 = NULL,
#' @examples          xh = NULL, Dis=rLOGNO2,l=c(exp, exp),
#' @examples          sigma = c(0.4, 0.3), sigmah = NULL,  sigmae = NULL,
#' @examples          costh = NULL, seed = 1234)
#' @examples #
#' @examples #b) Dagum data
#' @examples #
#' @examples data_gen(Ni=rep(10,4), D=4, M=2 ,ty =NULL,
#' @examples          k=c(3, log(3.4), -0.4, log(1)), b1=1.5,
#' @examples          x1=rnorm(40, 0,1), b2 = 0, x2 =NULL,
#' @examples          b3 = 0.1, x3 = rnorm(40, 0,1), b4 = 0, x4 = NULL,
#' @examples          xh = NULL, Dis=rGB2, l=c(exp, exp, exp ,exp),
#' @examples          sigma = c(0.15,0, 0, 0), sigmah = NULL, sigmae = NULL,
#' @examples          costh = NULL, seed = 1234)
...
#' @references Ramirez-Aldana, R., & Naranjo, L. (2021). Random intercept and linear mixed models including heteroscedasticity in a logarithmic scale: Correction terms and prediction in the original scale. PloS one, 16(4), e0249910.
#' @references Rigby, R. A., & Stasinopoulos, D. M. (2005). Generalized additive models for location, scale and shape. Journal of the Royal Statistical Society: Series C (Applied Statistics), 54(3), 507–554.
#' @author Lorenzo Mori and Maria Rosaria Ferrante

data_gen=function(Ni,M,D,k,b1,x1, b2=NULL,x2=NULL, b3=NULL,x3=NULL, b4=NULL,x4=NULL,
                  xh=NULL,Dis, l, sigma=NULL,sigmah=NULL, sigmae=NULL, ty=NULL,costh=NULL,
                  seed=NULL, id=NULL){
  list.data=list()
  if (is.null(seed)==T){seed=123}
  if (is.null(ty)==T){ty="other"}
  if (is.null(costh)==T){costh=1}
  if (is.null(id)==T){id=c(1:sum(Ni))}

  if (is.null(x2)==T & is.null(x3)==T & is.null(x4)==T ){
    covariate=as.data.frame(x1)
  } else if ( is.null(x3)==T & is.null(x4)==T ){
    covariate=as.data.frame(cbind(x1,x2))
    covariate=subset(covariate, select=which(!duplicated(names(covariate))))
  } else if ( is.null(x4)==T ){
  ...
  if(is.null(x2)==T){x2=rep(0, sum(Ni))}
  if(is.null(x3)==T){x3=rep(0, sum(Ni))}
  if(is.null(x4)==T){x4=rep(0, sum(Ni))}
  ...
  for (i in 1:M) {
    ...
    data=data.frame("y"=ym[,i], covariate, "sa"=as.factor(sort(rep(seq(1:D),Ni))),
                    "id"=id)
    list.data[[i]] <-data
  }
