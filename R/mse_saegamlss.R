...
#' @examples #
#' @examples #sample data with a sample fraction of 0.5
#' @examples #
#' @examples library(splitstackshape)
#' @examples #sample data
#' @examples #
#' @examples sample=stratified(data, "sa", size=0.5)
#' @examples #nonsample data
#' @examples #
#' @examples nonsample=nonsample(data=data, sample=sample, id=id)
#' @examples #estimate
#' @examples est=est_saegamlss(sample=sample, nonsample=nonsample, D=4,
#' @examples                  Ni=rep(10,4), ni=rep(1,4), f1=y~x1+random(sa),
#' @examples                  f2=NULL, f3=NULL, f4=NULL, fdis=NO, nRS=150,
#' @examples                  nCG=150, R=2, Dis=rNO, np=2, param=NULL,
#' @examples                  seed=1234, tau.fix=NULL, nu.fix=NULL)
#' @examples #
#' @examples #covariates
#' @examples #
#' @examples x=data.frame(rep(1, nrow(data)), "x1"=data$x1)
#' @examples #
#' @examples #compute the MSE
#' @examples #
#' @examples MSE=mse_saegamlss(est=est, D=4, Ni=rep(10,4), loop=2,
#' @examples                l=c(identity), Dis=rNO, Iden = TRUE,
#' @examples                samplesize=0.1, data=data, cov1=x, cov2 = NULL, cov3 = NULL,
#' @examples                cov4 = NULL, seed=1234)
#' @examples MSE$est_mse$MSE_mean
#' @examples MSE$est_mse$MSE_HCR
...
replace_term <- function(f, old, new){
  ...
  for (i in 1:loop) {
    ...
    if(est$input_var$np>1) {sigs=gamlss::getSmo(gam, what="sigma")$sigb} else{ sigs=0}
    if(est$input_var$np>2) {sign=gamlss::getSmo(gam,what="nu")$sigb} else{ sign=0}
    if(est$input_var$np>3) {sigt=gamlss::getSmo(gam,what="tau")$sigb} else{ sigt=0}

    if (is.null(sigs)==TRUE){sigs=0}
    if (is.null(sign)==TRUE){sign=0}
    if (is.null(sigt)==TRUE){sigt=0}
    ...
    if(est$input_var$np>1){
      ...
      if (setequal(c(as.character(est$input_var$f2)),def)==TRUE ){
        ...
        } else {sigma_star= stats::fitted(gam, "sigma")[1] }
      } else{
        if (Iden==TRUE){
    ...
    estMSE=est_saegamlss(sample=s, nonsample = ns,
