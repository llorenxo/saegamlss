...
#'
#' @description The function selects the final SAE-GAMLSS model following three steps as described by Mori and Ferrante (2024).
#'
#' @param sample The dataset with sampled units
...
#'
#'
#' sol <- m_selection(sample = s_data, y = "y",
#'                    f_cov = ~x1+x2+x3+random(sa), ndis = 1,
#'                    R = 2, k = 2, type = "B", fix_dis = "NO",
#'                    seed = 123, kp = 2)
#'
#' #Step 1
#'
...
m_selection <- function(sample, y, f_cov, kp = 2,
  ...
  s_data <- modify_data(data = sample, y = y)
